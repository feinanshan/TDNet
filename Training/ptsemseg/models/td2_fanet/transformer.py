...
class Encoding(nn.Module):
    def __init__(self, d_model, d_k, d_v, norm_layer=None, dropout=0.1):
        ...
        self.maxpool_k = nn.MaxPool2d(kernel_size=1, stride=3, padding=0)
        self.maxpool_v = nn.MaxPool2d(kernel_size=1, stride=3, padding=0)

    def forward(self, fea, pre=None):
        ...
        if pre:
            k = self.w_ks(fea).view(n_, d_k, h_, w_)
